...
Talisman(
    ...
    strict_transport_security=os.getenv("STRICT_TRANSPORT_SECURITY", "False").lower()
    == "true",
...
if not app.debug:
    ...
    file_handler = RotatingFileHandler(
        "logs/securevault.log", maxBytes=10240000, backupCount=10
    )
...
def signup():
    if request.method == "POST":
        ...
        is_valid_password, password_error = validators.validate_password_strength(
            password
        )
            ...
            try:
                ...
                if email_sent:
                    ...
                    if os.getenv("FLASK_DEBUG", "False").lower() == "true":
                        ...
                        db.commit()
                        app.logger.info(
                            "⚠️ DEV MODE: Auto-verified user due to email failure"
                        )
                        flash(
                            "Account created successfully! (Email verification bypassed in dev mode)",
                            "success",
                        )
                    else:
                        flash(
                            "Account created but verification email failed to send. Please try resending the verification email from the login page.",
                            "warning",
                        )
                ...
                if os.getenv("FLASK_DEBUG", "False").lower() == "true":
                    ...
                    flash(
                        "Account created but verification email failed. Please try resending the verification email from the login page.",
                        "warning",
                    )
...
@login_required
def dashboard():
    # Preload passwords to ensure they're available immediately on page load
    with get_session() as db:
        result = db.execute(
            select(Password).where(Password.user_id == session["user_id"])
        )
    ...
    password_list = [
        {
            ...
            "password": security.decrypt_password(
                base64.b64decode(pwd.encrypted_password)
            ),
        }
        for pwd in passwords
    ]

    return render_template("dashboard.html", initial_passwords=password_list)
...
def create_password():
    ...
    is_valid_website, website_error = validators.validate_website_name(
    ...
    # Validate username
    is_valid_username, username_error = validators.validate_username_field(
        data.get("username", "")
    )
    ...
    with get_session() as db:
        result = db.execute(
            select(Password).where(
                Password.id == password_id, Password.user_id == session["user_id"]
            )
        )
                ...
                try:
                    ...
                    if email_sent:
                        ...
                        if os.getenv("FLASK_DEBUG", "False").lower() == "true":
                            ...
                            flash(
                                "Account verified successfully! (Email verification bypassed in dev mode)",
                                "success",
                            )
                        else:
                            flash(
...
def verify_2fa():
    ...
    if request.method == "POST":
        ...
        with get_session() as db:
            result = db.execute(
                select(MFATOTP).where(MFATOTP.user_id == session["temp_user_id"])
            )
            mfa = result.scalar_one_or_none()
...
def get_2fa_status():
    with get_session() as db:
        result = db.execute(
            select(MFATOTP).where(MFATOTP.user_id == session["user_id"])
        )
        mfa = result.scalar_one_or_none()
    ...
    try:
        ...
        with get_session() as db:
            for pwd_data in passwords:
                # Validate required fields
                if not all(
                    key in pwd_data for key in ["website_name", "username", "password"]
                ):
                    continue
                ...
                if existing:
                    ...
                    existing.encrypted_password = base64.b64encode(
                        encrypted_pwd
                    ).decode("utf-8")
                else:
                    ...
                    new_password = Password(
                        ...
                        encrypted_password=base64.b64encode(encrypted_pwd).decode(
                            "utf-8"
                        ),
                    )
                    db.add(new_password)
